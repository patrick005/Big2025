...
int main(int argc, char *argv[]){
    ...
    if (argc != 2){//----------------------------------------------------------
        printf("사용법: %s <port>\n", argv[0]);//----------------------------------------------------------이거 인자수에 따라 다른곳 변경해야해서 와드 꽂아둠
        exit(1);
    }
    ...
    mul_addr.sin_port = htons(atoi(argv[1])); // 멀티캐스트 포트 //----------------------------------------------------------
    ...
    setsockopt(sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&mul_addr, sizeof(mul_addr));
