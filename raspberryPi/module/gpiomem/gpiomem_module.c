...
#define GPIO_BASE 0xFE200000 //데이터시트는 테스트용 cpu라 조금 다름
...
static void __iomem *gpio_base;//가상 메모리 주소 설정

static int led_module_init(void){
    int i, reg_offset, bit;
    __u32 value; //언사인 32비트랑 비슷한거임 커널에서는 언더바 두개 붙이고 지정해줌 os마다 다름
    gpio_base = ioremap(GPIO_BASE, GPIO_SIZE); //가상 메모리 주소 할당
