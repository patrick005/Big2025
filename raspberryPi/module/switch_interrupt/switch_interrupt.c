...
irq_handler_t irq_set_handler(int irq, void *dev_id);
...
static int switch_interrupt_init(void){
    ...
    for(i = 0; i<4; i++){
        ...
        ret = request_irq(gpio_to_irq(sw[i]),(irq_handler_t)irq_set_handler, IRQF_TRIGGER_RISING, "IRQ", (void *)(irq_set_handler));
        if(ret<0){
...
static void switch_interrupt_exit(void){
    ...
    for(i=0; i<4; i++){
        free_irq(sw[i], (void *)irq_set_handler);
        gpio_free(sw[i]);
    }
