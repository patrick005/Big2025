...
// 스위치를 누르고 떼면 -> FND에 숫자 출력 (0~1초) -> (00~99)||ms -> 10ms 단위
...
void SW_ISR1R(void){
    if (!r_pushed){
        ...
        bit_74HC595_shift16(numbers[(int)((float)r_value / 409.6)], numbers[(int)((float)r_value / 40.96)%10]);
        r_pushed = 0;
    }
}
...
void SW_ISR2R(void){
    if (!g_pushed){
        ...
        printf("G: %d\n", g_value);
        bit_74HC595_shift16(numbers[(int)((float)g_value / 409.6)], numbers[(int)((float)g_value / 40.96)%10]);
        g_pushed = 0;
    }
}
...
void SW_ISR3R(void){
    if (!b_pushed){
        ...
        printf("B: %d\n", b_value);
        bit_74HC595_shift16(numbers[(int)((float)b_value / 409.6)], numbers[(int)((float)b_value / 40.96)%10]);
        b_pushed = 0;
    }
}